...
model_type="linear"; NFIT=3; X_KEYS=['x1','x2']; Y_KEYS=['y']

#SAVE HISTORY FOR PLOTTING AT THE END
epoch=1; epochs=[]; loss_train=[];  loss_val=[]

#------------------------
#GENERATE(Read in) DATA
#------------------------
...
def minimizer(f,xi, algo='GD', LR=0.01):
	...
	while(iteration<=max_iter):
		...
		for i in range(0,NDIM):	#LOOP OVER DIMENSIONS
			...
			delta_model_parameters = np.zeros(NDIM) # initialize delta_model_parameters
		...
		if(algo=="MOM"): xip1 =  xi - LR*df_dx - decay_factor * delta_model_parameters
		...
		if iteration == 1:
			...
			delta_model_parameters += decay_factor * LR * df_dx
...
p_final=minimizer(loss,po,algo='MOM')		
