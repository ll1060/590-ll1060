...
def NN_eval(x_train, weights_bias_matrices):
    for i in range(0, len(weights_bias_matrices), 2):
#         print(i)
        if i == 0:
            intermediate_layer_results = np.matmul(weights_bias_matrices[i], x_train.T) + weights_bias_matrices[i+1]
#             print(intermediate_layer_results.shape)
        else:
#             print(weights_bias_matrices[i].shape)
#             print(intermediate_layer_results.shape)
            intermediate_layer_results = np.matmul(weights_bias_matrices[i], intermediate_layer_results) + weights_bias_matrices[i+1]
    return intermediate_layer_results.T
